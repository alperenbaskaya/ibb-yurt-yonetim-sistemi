...
export function DormitoryHistoryPage() {
  ...
  const historyQuery = useDormitoryAdminHistory(userId, category, page, debouncedSearch)

  useEffect(() => {
    if (historyQuery.data && page >= historyQuery.data.totalPages && historyQuery.data.totalPages > 0) {
      setPage(historyQuery.data.totalPages - 1)
    }
  ...
  return <section>
    ...
    <div role="tabpanel">
      ...
      {historyQuery.isSuccess && historyQuery.data.items.length === 0 && <AdminPageState state="empty" title={search ? 'Aramanızla eşleşen geçmiş kaydı bulunamadı' : emptyStudent ? 'Öğrenci işlem geçmişi bulunmuyor' : 'Değerlendirici işlem geçmişi bulunmuyor'} message={search ? 'Arama metnini değiştirerek tekrar deneyin.' : emptyStudent ? 'Bu yurtta henüz öğrenci işlem geçmişi bulunmuyor.' : 'Bu yurtta henüz değerlendirici işlem geçmişi bulunmuyor.'} />}
      {historyQuery.isSuccess && historyQuery.data.items.length > 0 && <div className="mt-6"><p className="mb-3 text-sm text-slate-600">Toplam {historyQuery.data.totalElements} kayıt</p><ol className="space-y-3">{historyQuery.data.items.map((event) => <DormitoryHistoryCard key={event.id} event={event} />)}</ol><Pagination currentPage={historyQuery.data.page} totalPages={historyQuery.data.totalPages} totalElements={historyQuery.data.totalElements} pageSize={historyQuery.data.size} onPageChange={setPage} itemLabel="kayıt" className="mt-5" /></div>}
    </div>
  </section>
}
