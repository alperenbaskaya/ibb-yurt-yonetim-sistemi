...
export const globalAdminKeys = {
  ...
  admissionsRoot: (u:number) => [...globalAdminKeys.root(u),'admissions'] as const,
  admissions: (u:number,page:number,status:AdmissionStatus|undefined) => [...globalAdminKeys.admissionsRoot(u),page,status??'ALL'] as const,
...
export const useGlobalAdmissions=(u:number,page:number,status?:AdmissionStatus)=>useQuery({queryKey:globalAdminKeys.admissions(u,page,status),queryFn:()=>api.getGlobalAdmissions(page,status)})
...
export function useAdmissionMutations(u:number){const inv=useInvalidate();const refresh=()=>inv([globalAdminKeys.admissionsRoot(u),globalAdminKeys.dashboard(u),globalAdminKeys.studentsRoot(u)]);const conflictRefresh=async(e:unknown)=>{if(axios.isAxiosError(e)&&e.response?.status===409)await refresh()};return {single:useMutation({mutationFn:({id,status}:{id:number;status:AdmissionStatus})=>api.updateGlobalAdmissionStatus(id,{status}),onSuccess:refresh,onError:conflictRefresh}),selected:useMutation({mutationFn:api.approveSelectedGlobalAdmissions,onSuccess:refresh,onError:conflictRefresh}),all:useMutation({mutationFn:api.approveAllPendingGlobalAdmissions,onSuccess:refresh,onError:conflictRefresh})}}
